...
- WPF with .NET 10's **native Fluent theme** (`<ThemeMode>System</ThemeMode>`, `PresentationFramework.Fluent`) plus `DwmSetWindowAttribute` interop for the acrylic backdrop and rounded corners. **Supersedes the original WPF-UI (lepo.co) choice** (decided 2026-08-08): the framework now covers what WPF-UI was for, so the shell keeps one less third-party dependency and a smaller exe. Acrylic degrades to a solid brush where DWM refuses it.
...
- FlyoutWindow: borderless, positioned above tray corner from `Shell_NotifyIconGetRect`, light-dismiss on `Deactivated`, acrylic backdrop via `DwmSetWindowAttribute` **with a solid-color fallback** (acrylic availability is Win11-version-sensitive).
