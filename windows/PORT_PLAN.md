...
- `Microsoft.Toolkit.Uwp.Notifications` (`ToastNotificationManagerCompat`) for toasts - works in unpackaged single-file apps with no extra runtime. NOTE: package is archived/maintenance-mode; a Phase 6 spike de-risks it (see Phases). Fallback if the spike fails: Windows App SDK `AppNotificationManager` with `WindowsAppSDKSelfContained`.
...
- **Entry spike (do first): toast activation on unpackaged single-file exe.** Publish a hello-world single-file WPF exe, fire a toast via `ToastNotificationManagerCompat` with launch args, click it with the app running AND not running, confirm `OnActivated` fires with args both ways. If this fails, switch the toast stack to the WASDK fallback *now*, before any ToastService code exists.
